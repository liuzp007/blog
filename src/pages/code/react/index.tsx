    ...
    >
      <div className="mb-12 grid gap-6 [grid-template-columns:repeat(auto-fit,minmax(250px,1fr))]">
        {concepts.map((concept, index) => (
          <div
            key={index}
            className="relative z-[3] cursor-pointer rounded-xl border border-[color-mix(in srgb, var(--color-indigo-500) 30%, transparent)] bg-[color-mix(in srgb, var(--color-indigo-500) 10%, transparent)] p-6 transition-all duration-300 hover:-translate-y-[5px] hover:bg-[color-mix(in srgb, var(--color-indigo-500) 20%, transparent)] hover:shadow-[0_10px_25px_color-mix(in srgb, var(--color-indigo-500) 30%, transparent)]"
          >
            <div className="mb-2 text-[2.5rem]">{concept.icon}</div>
            <h3 className="mb-2 text-[1.2rem] text-[var(--color-code-indigo)]">{concept.title}</h3>
            <p className="m-0 leading-[1.6] text-[color-mix(in srgb, var(--color-white) 70%, transparent)]">
              {concept.description}
            </p>
          </div>
        ))}
      ...
      <div className="relative z-[3] rounded-xl border border-[color-mix(in srgb, var(--color-code-violet) 30%, transparent)] bg-[color-mix(in srgb, var(--color-code-violet) 10%, transparent)] p-6">
        <h3 className="mb-6 text-[1.5rem] text-[var(--color-code-violet)]">🎣 React Hooks</h3>
        <div className="grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(200px,1fr))]">
          {hooksList.map((hook, index) => (
            <div
              key={index}
              className="rounded-lg border border-[color-mix(in srgb, var(--color-code-violet) 20%, transparent)] bg-[color-mix(in srgb, var(--color-code-violet) 5%, transparent)] px-4 py-3 transition-all duration-200 [font-family:var(--font-family-mono)] text-[var(--color-code-violet)] hover:border-[color-mix(in srgb, var(--color-code-violet) 40%, transparent)] hover:bg-[color-mix(in srgb, var(--color-code-violet) 15%, transparent)]"
            >
              <div className="mb-1 font-bold">{hook.name}</div>
              <div className="text-[0.8rem] text-[color-mix(in srgb, var(--color-white) 60%, transparent)]">
                {hook.description}
              </div>
            </div>
          ))}
      ...
      <div className="relative z-[3] mt-6 rounded-xl border border-[color-mix(in srgb, var(--color-code-green) 30%, transparent)] bg-[color-mix(in srgb, var(--color-code-green) 10%, transparent)] p-6">
        <h3 className="mb-4 text-[1.5rem] text-[var(--color-code-green)]">📚 学习路径</h3>
        <div className="flex flex-col gap-4 text-[color-mix(in srgb, var(--color-white) 80%, transparent)]">
          <div className="flex items-center gap-4">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[color-mix(in srgb, var(--color-code-green) 20%, transparent)] font-bold">
              1
            </span>
            <div>
